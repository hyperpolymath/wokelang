...
let showInfo = () => {
  let _ = Window.showInformationMessage("WokeLang LSP extension v0.1.0 - Consent-aware programming language")
  ()
}

let checkConsent = () => {
  let _ = Window.showInformationMessage("Consent safety check: Analyzing capability usage patterns...")
  ()
}

let formatDocument = () => {
  let _ = Window.showInformationMessage("Formatting with WokeLang formatter...")
  ()
}
...
let activate = (context: extensionContext) => {
  ...
  let infoCmd = Commands.registerCommand("wokelang.showInfo", () =>
    Js.Promise.make((~resolve, ~reject as _) => {
      showInfo()
      resolve(.)
    })
  )
  let consentCmd = Commands.registerCommand("wokelang.checkConsent", () =>
    Js.Promise.make((~resolve, ~reject as _) => {
      checkConsent()
      resolve(.)
    })
  )
  let formatCmd = Commands.registerCommand("wokelang.formatDocument", () =>
    Js.Promise.make((~resolve, ~reject as _) => {
      formatDocument()
      resolve(.)
    })
  ...
  let _ = Js.Array2.push(context.subscriptions, consentCmd)
