...
This document provides the category-theoretic foundations underlying
WokeLang’s type system and semantics.
...
* *Objects:* Types (Int, Float, String, Bool, Unit, [τ], Maybe τ,
Result[τ,ε], (τ₁,…,τₙ) → τ)
...
WokeLang doesn’t have an explicit void type, but runtime errors can be
viewed as ⊥.
...
For fixed error type ε, `+Result[-, ε] : WokeType → WokeType+` is a
functor:
...
*Definition:* An initial F-algebra is an F-algebra (μF, in : F(μF) → μF)
such that for any F-algebra (A, α), there exists a unique morphism
(catamorphism) ⦇α⦈ : μF → A.
...
A natural transformation η : F ⟹ G between functors F, G : C → D is a
family of morphisms:
...
Solved using: 1. Bilimit construction 2. Information systems 3. Inverse
limit construction
...
Linear types ensure resources (like Result values) are used exactly
once:
...
. Barr, M. and Wells, C. (1990). "`Category Theory for Computing
Science`"
