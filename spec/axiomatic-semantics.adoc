...
WokeLang’s axiomatic semantics extend Hoare logic with _consent
preconditions_ — formal requirements that certain operations may only
proceed if explicit user consent has been obtained. This ensures that
WokeLang programs cannot perform sensitive operations silently.
...
`+thanks to+` block. This is a convention, not enforced by the type
system.
